...
class CoSiamRPNTracker(BaseTracker):
    def __init__(self, model):
        ...
        self.img_norm_cfg = dict(mean=np.array([0.485, 0.456, 0.406],dtype=np.float32),
    ...
    def track(self, img):
        ...
        # Fuse outputs
        score = self._norm_score(pcls) * (1 - cfg.WINDOW_INFLUENCE) + cfg.WINDOW_INFLUENCE * self.window + \
                cfg.MAN_INFLUENCE * self._norm_score(response)
    ...
    def get_subwindow(self, im, pos, model_sz, original_sz, avg_chans):
        ...
        if self.model.backbone_type is not 'resnet50c3':
            im_patch = self.imnormalize(im_patch, **self.img_norm_cfg)
