...
parser.add_argument('--reuse', default=0, type=int)
parser.add_argument('--cw', default='0.0,0.401,0.1', type=parse_range) # 0.3,0.45,0.05
parser.add_argument('--pk', default='0.2,0.3,0.05', type=parse_range)
parser.add_argument('--mi', default='0.0,0.001,0.05', type=parse_range) # 0.2,0.4,0.1
parser.add_argument('--wi', default='0.25,0.401,0.05', type=parse_range)
parser.add_argument('--lr', default='0.25,0.255,0.1', type=parse_range)
...
if __name__ == '__main__':
    snap_shot = './checkpoints/eff/model_e19.pth'

    torch.cuda.set_device(args.gpuid)
    args.wi = np.array([0.05 * args.reuse]) + 0.25

    num_search = len(args.cw) * len(args.pk) * len(args.wi) * len(args.mi) * len(args.lr)
    ...
    for idx in seqs:
        ...
        # shuffle hp-params
        np.random.shuffle(args.cw)
        np.random.shuffle(args.pk)
        np.random.shuffle(args.wi)
        np.random.shuffle(args.mi)
            ...
            for pk in args.pk:
                for wi in args.wi:
                    for mi in args.mi:
                        for lr in args.lr:
                            # update hp-params
                            cfg.CO_WEIGHT = float(cw)
                            cfg.PENALTY_K = float(pk)
                            cfg.MAN_INFLUENCE = float(mi)
