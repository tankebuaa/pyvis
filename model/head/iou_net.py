...
class IoUNet(nn.Module):
    def __init__(self, input_dim=512, pred_input_dim=256, pred_inter_dim=256):
        super().__init__()
        # _r for reference, _t for test
        self.conv_1r = conv(input_dim, 128, kernel_size=3, stride=1)
        self.conv_1t = conv(input_dim, 256, kernel_size=3, stride=1)

        self.conv_2t = conv(256, pred_input_dim, kernel_size=3, stride=1)

        self.prroi_pool3r = PrRoIPool2D(3, 3, 1 / 8)
        self.prroi_pool3t = PrRoIPool2D(5, 5, 1 / 8)

        self.fc3_r = conv(128, 256, kernel_size=3, stride=1, padding=0)

        self.fc34_3r = conv(256, pred_input_dim, kernel_size=1, stride=1, padding=0)

        self.fc3_rt = LinearBlock(pred_input_dim, pred_inter_dim, 5)

        self.iou_predictor = nn.Linear(pred_inter_dim, 1, bias=True)
    ...
    def predict_iou(self, modulation, feat, proposals):
        ...
        iou_pred = self.iou_predictor(fc3_rt).reshape(batch_size, num_proposals_per_batch)
    ...
    def get_iou_feat(self, feat2):
        ...
        c3_t = self.conv_2t(self.conv_1t(feat2))
        return c3_t
