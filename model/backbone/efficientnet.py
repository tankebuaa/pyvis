...
class EfficientNet(nn.Module):
    ...
    def train(self, mode=True):
        ...
        if mode:
            # freeze all layers
            self.eval()
            # un-freeze training blocks
            for block in self._blocks[self.frozen_blocks:]:
                block.train(mode)

        if not self._is_frozen_nograd:
            # set all layers to nograd
            for p in self.parameters():
                p.requires_grad_(False)
            # set training blocks to grad
            for block in self._blocks[self.frozen_blocks:]:
    ...
    def activate_training_layers(self, mode=True):
        block = self._blocks[self.frozen_blocks-1]
        for p in block.parameters():
...
def EfficientNetB0(pretrained=True, frozen_blocks=5):
    ...
    if pretrained:
        load_pretrained_weights(model, 'efficientnet-b0', weights_path=None, load_fc=False, advprop=False)
...
if __name__ == '__main__':
    ...
    inputs = torch.rand(1, 3, 127, 127).cuda()
    model = EfficientNetB0(pretrained=True, frozen_blocks=5).cuda()
    model.train()
    output = model(inputs)
