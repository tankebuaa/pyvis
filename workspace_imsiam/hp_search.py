...
parser.add_argument('--penalty-k', default='0.0,0.005,0.05', type=parse_range)
parser.add_argument('--lr', default='0.25,0.55,0.1', type=parse_range)
parser.add_argument('--window-influence', default='0.2,0.5,0.05', type=parse_range)
...
if __name__ == '__main__':
    snap_shot = './checkpoints/3rd/model_e19.pth'
    torch.cuda.set_device(args.gpuid)
    win = 8 - args.gpuid
    args.window_influence = np.array([0.1 * win, 0.1 * win + 0.05]) + 0.1
            ...
            for wi in args.window_influence:
                for lr in args.lr:
                    ...
                    tracker_path = os.path.join(benchmark_path, (model_name + '_wi-{:.3f}'.format(wi) +
                                                                 '_pk-{:.3f}'.format(pk) + '_lr-{:.3f}'.format(lr)))
